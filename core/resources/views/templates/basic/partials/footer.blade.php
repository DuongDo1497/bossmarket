@php
  ...
  $categories = \App\Models\Category::where('is_top', 1)
      ...
      ->take(6)
      ->get();
  $topBrands = \App\Models\Brand::top()
      ->inRandomOrder()
      ->take(6)
      ->get();
              ...
              <div class="footer__widget">
                ...
                <ul class="footer__links">
                  @foreach ($topBrands as $brand)
                    <li>
                      <a
                        href="{{ route('products.brand', [$brand->id, slug($brand->name)]) }}">{{ $brand->name }}</a>
                    </li>
                  @endforeach
                ...
                <ul class="footer__links">
                  @if ($pages->count() > 0)
                    @foreach ($pages as $item)
                      <li><a
                          href="{{ route('page.details', [$item->id, slug($item->data_values->pageTitle)]) }}">@php echo __($item->data_values->pageTitle) @endphp</a>
                      </li>
                    @endforeach
                  @endif
                </ul>
      ...
      <div class="d-flex flex-wrap justify-content-around justify-content-lg-between align-items-center">
        ...
        <div class="right py-2">
          @isset($footer->payment_methods)
            <img src="{{ getImage('assets/images/frontend/footer/' . @$footer->payment_methods, '250x30') }}"
              alt="@lang('footer')">
          @endisset
        </div>
      </div>
